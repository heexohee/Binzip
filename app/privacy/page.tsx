  ...
  return (
    <div className={home.site}><div className={styles.sheet}><HomeHeader /><main className={styles.main}>
      ...
      <Section no="②" title="무엇에 씁니까">
        <P>주소와 지도 위치를 확인하고, 상세 정보를 보내 진단을 신청하시면 검토한 진단서를 보내드리는 데 씁니다.</P>
        <P>메인의 주소 확인에는 입력한 주소만 사용합니다. 지도 표시를 위해 주소를 좌표로 조회하고 해당 좌표의 위성·항공사진을 불러옵니다. 상세 신청 전에는 연락처·이메일을 받지 않고 신청·진단서 데이터베이스에 기록을 만들거나 메일을 보내지 않습니다. 입력한 주소와 확인 결과는 열린 화면의 메모리에 머물며 새로고침하거나 창을 닫으면 초기화됩니다. 공공자료의 중복 조회를 줄이기 위해 서버 메모리에서 24시간 유효한 캐시를 사용합니다.</P>
