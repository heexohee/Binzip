...
import { DECISIONS, DECISION_ACTIONS, type Decision, type DecisionAction } from '../../src/decision-actions'
import { resourceById } from '../../src/official-resources'
import { CopyQuestion } from './report-tools'
import styles from './report.module.css'

export function DecisionActions({ attention = [], initialDecision = 'undecided', onDecisionChange, actions = DECISION_ACTIONS }: { attention?: string[]; initialDecision?: Decision; onDecisionChange?: (decision: Decision) => void; actions?: Record<Decision, readonly DecisionAction[]> }) {
  const group = useId()
  ...
  return <section id="next-actions" className={styles.section} aria-labelledby="actions-title">
    ...
    <ol className={styles.steps} key={decision}>{actions[decision].map((step, index) => <li key={step.id}><span className={styles.stepNumber}>{index + 1}</span><div><p className={styles.stepWhere}>{step.where}</p><h3>{step.title}</h3><p className={styles.stepDetail}>{step.detail}</p>
