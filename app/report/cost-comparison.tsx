...
export function CostComparison({ example = false }: { example?: boolean }) {
  ...
  return <section id="money" className={styles.money} aria-labelledby="money-title">
    <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>01 · 돈부터 비교해요</p><h2 id="money-title">팔고 나면, 얼마가 남을까요?</h2></div>{!example && <span className={styles.tag}>직접 입력 계산</span>}</div>
    <p className={styles.intro}>{example ? '팔아서 받는 돈에서 갚을 돈과 정리 비용을 빼면, 내게 남는 돈을 알 수 있어요.' : '중개사·금융기관·세무사·철거업체에서 확인한 금액을 넣어 비교해 보세요. 아직 모르는 금액은 비워 두세요.'}</p>
      ...
      {OPTIONS.map(option => {
        ...
        return <article className={styles.moneyCard} key={option.title}>
          ...
          <div className={styles.total} aria-live="polite" aria-atomic="true">
            ...
            {(result.net === null || !example) && <span>{result.net === null ? `${result.missing.length + result.invalid.length}개 항목을 확인해 주세요` : '직접 입력한 금액 기준 · 검증 전'}</span>}
          </div>
    ...
    <details className={styles.editor}>
      <summary>{example ? '금액을 바꿔서 계산해 보기' : '확인한 금액 입력하기'}<span aria-hidden="true">＋</span></summary>
      <div className={styles.editorContent}>
        ...
        <div className={styles.editorActions}><button type="button" onClick={() => setInputs([emptyDisposal(), emptyDisposal()])}>입력값 비우기</button>{example && <button type="button" onClick={() => setInputs(initialDisposal(true))}>처음 금액으로</button>}</div>
      </div>
    </details>
