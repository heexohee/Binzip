...
- 발표 메모: [빈집의 문제의식과 다음 결정을 돕는 서비스 (2026-09-12)](docs/발표메모-2026-09-12-빈집과-다음결정.md)
- 디자인: [디자인 시스템](docs/design-system.md) · [화면별 불일치와 수정 우선순위](docs/design-audit-2026-09-12.md)
- **마감 2026-09-18** · 현재 **W1(데이터)** 구간

홈에서 주소 입력 → 조회된 주소·위성 지도에서 위치 확인 → `/apply`에서 집 상태·소유관계·사진·연락처 입력 → 진단 신청 흐름을 제공한다. 이전 `/diagnose` 주소는 메인의 주소 확인 단계로 이동한다. [현재 주소 확인 흐름과 검증](docs/address-intake-flow.md)을 참고한다.
...
세움터에서 포항 남구(47111)·북구(47113) 건축물대장(표제부) 대량자료를 받아 `data/raw/` 에 둔다.
...
`data/g1-addresses.txt` 에 구룡포·호미곶(남구) 실제 주소 10개를 적고:
...
- **산여부 코드가 두 체계다.** PNU 는 `1=일반 / 2=산`, 건축물대장 `platGbCd` 는 `0=대지 / 1=산`.
  구룡포읍·호미곶면(남구)은 산지 필지가 많아 반드시 걸린다. `src/pnu.ts` 가 둘 다 만들어 준다.
...
`/admin-preview`에서 가상 신청으로 관리자 화면을 확인하고, `/admin/login`에서 운영자 접근 키로 실제 신청에 접근합니다. 연결 설정·DB 업데이트·공개 범위는 [관리자 화면 문서](docs/admin-workbench.md)를 참고하세요.

- [디자인 시스템 적용·반응형 검증](docs/design-system-implementation-2026-09-12.md)
