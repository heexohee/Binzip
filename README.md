...
홈의 주소·고민 입력 → `/diagnose`에서 주소 확인 → 공적 기록 기반 1차 진단서 즉시 표시 → 필요시 `/apply`에서 사진·추가 확인 신청 흐름을 제공한다. 1차 조회의 범위, 실패 처리와 로컬 검증은 [즉시 진단 안내](docs/instant-diagnosis.md)를 참고한다.
